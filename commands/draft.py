...
async def draft(message, args):
  ...
  roster = service.spreadsheets().values().get(spreadsheetId=sheet, range='Rosters', majorDimension='COLUMNS').execute()
  roster = roster.get('values', [])[result[1]*3+2]
  msg = '\n'.join([r for r in roster if r != None])
