...
async def badge(message, args):
  if haspermission(message.author):
    ...
    cursor.execute(open_challenge_badge_select_str, (int(message.timestamp.timestamp()), user.id, badge))
    result = cursor.fetchone()
    if result == None:
      msg = 'User does not have a matching challenge')
    else:
      cursor.execute(challenge_win_str, (user.id, badge))
      connection.commit()
      msg = 'Assigned ' + badge + ' badge to ' + discorduser_to_discordname(user))
  else:
    await msg = no_permissions_message
